...
Memory Usage: 22.7 MB, less than 7.41% of C++ online submissions for Diameter of Binary Tree.
...
public:
    ...
    int getDiameter(TreeNode* root){
        ...
        while(!q.empty()){
            ...
            int diam_left = -1;
            if(node->left != NULL){
                diam_left = node_diam[node->left];
                q.push(node->left);
            }
            int diam_right = -1;
            if(node->right  != NULL){
                diam_right = node_diam[node->right];
                q.push(node->right);
            }
            int cur_diam = diam_left + diam_right + 2;
            if(cur_diam > result){
                result = cur_diam;
            }
        }
